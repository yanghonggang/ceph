...
void test_object_list(object_store_t os, collection_t coll) {
  ...
  while (1) {
    int ret = os_object_list(os, coll, start, end, 10, buf, &buf_len, &count,
                             next, &next_len);
    if (ret < 0) {
...
void test_rename(object_store_t os, config_ctx_t ctx) {
  ...
  int ret =
      os_object_read(os, coll, oldoid, 0, sizeof(read_buffer), read_buffer, 0);
  if (ret < 0) {
    fprintf(stderr,
            "[rename test before rename] os_object_read failed: %d (%s)\n", ret,
            strerror(-ret));
  } else {
  ...
  if (ret < 0) {
    fprintf(stderr,
            "[rename test after rename] os_object_read failed: %d (%s)\n", ret,
            strerror(-ret));
  } else {
  ...
  int ret = -1;
  {
...

    {
      cid_t cid = 12345;
      {
        ...
        printf("#Checking if collection with cid=%" PRIu64
               " exists using os_collection_list...\n",
               target_cid);
        ...
        if (!found) {
          fprintf(stderr,
                  "Collection with cid=%" PRIu64
                  " does not exist according to os_collection_list.\n",
                  target_cid);
        } else {
      ...
      char read_buffer[1024];
      memset(read_buffer, 0, sizeof(read_buffer));
      ret = os_object_read(os, coll2, oid, 0, sizeof(read_buffer), read_buffer,
                           0);
      if (ret < 0) {
        fprintf(stderr, "os_object_read failed: %d (%s)\n", ret,
                strerror(-ret));
...

    {
      ...
      if (ret < 0) {
        fprintf(stderr,
                "os_transaction_collection_remove failed in tx3: %d (%s)\n",
                ret, strerror(-ret));
        goto release_tx3;
      }
      ...
      char read_buffer[1024];
      memset(read_buffer, 0, sizeof(read_buffer));
      ret = os_object_read(os, coll3, oid, 0, sizeof(read_buffer), read_buffer,
                           0);
      if (ret < 0) {
        fprintf(stderr, "os_object_read failed: %d (%s)\n", ret,
                strerror(-ret));
