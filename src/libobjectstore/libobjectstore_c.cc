...
static CephContext *create_cct(const char *const clustername,
  ...
  cct->_conf.parse_env(
      cct->get_module_type()); // environment variables override
...
static inline cid_t get_cid_t(const coll_t &coll) { return coll.pool(); }
...
extern "C" collection_t os_create_new_collection(object_store_t os_,
                                                 cid_t cid_) {
  ObjectStore *os = static_cast<ObjectStore *>(os_);
  if (!os) {
    std::cerr << "os is null" << std::endl;
...
extern "C" int os_object_read(object_store_t os_, collection_t coll,
                              const char *oid, uint64_t offset, uint64_t len,
...
extern "C" int os_transaction_object_write(transaction_t tx, cid_t cid_,
                                           const char *oid, const char *data,
...
extern "C" int os_object_list(object_store_t os_, collection_t c,
                              const char *start, const char *end, int max,
                              char *buf, uint64_t *buf_len, int *count,
  ...
  *count = 0;
  ...
  int ret =
      os->collection_list(cc->ch, start_oid, end_oid, max, &ls, &next_ghobj);
  if (ret < 0) {
    return ret;
  }
  ...
  for (const auto &obj : ls) {
    ...
    std::cout << __func__ << " obj=" << obj << " | hobj=" << obj.hobj
              << "| key=" << key << std::endl;
