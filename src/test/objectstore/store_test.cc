...
#define WRITE_AT(offset, _length) {\
  ...
  g_conf->set_val("bluestore_compression_min_blob_size", "262144");
  // https://github.com/ceph/ceph/pull/23752
...
TEST_P(StoreTest, BluestoreInlineWriteReadDelete) {
  ...
  g_ceph_context->_conf->set_val("data_inline_limit_bytes", "20");
  g_ceph_context->_conf->set_val("debug_rocksdb", "22");
  g_ceph_context->_conf->set_val("debug_osd", "22");
  g_ceph_context->_conf->set_val("debug_bluestore", "22");
  g_conf->set_val("bluestore_fsck_on_mount", "true");
  g_conf->set_val("bluestore_fsck_on_umount", "true");
  ...
  ghobject_t hoid(hobject_t("test_inline_wrd", "", CEPH_NOSNAP, 0, 0, ""));
  {
    ...
    ObjectStore::Transaction t;
    t.create_collection(cid, 0);
    r = apply_transaction(bstore, &osr, std::move(t));
    ASSERT_EQ(r, 0);
  }
  {
    ObjectStore::Transaction t;
    t.touch(cid, hoid);
    t.set_alloc_hint(cid, hoid, 19, 19, CEPH_OSD_ALLOC_HINT_FLAG_INLINE);
  ...
  }
  {
    ...
    bufferlist readback;
    r = store->read(cid, hoid, 1, 10, readback);
    ASSERT_EQ(r, 10);
    {
      bufferlist expected;
  ...
  }
  {
    ...
    large.append("0123456789abcdefghi");
    {
      ...
      t.set_alloc_hint(cid, hoid2, large.length(), large.length(),
                       CEPH_OSD_ALLOC_HINT_FLAG_INLINE);
    ...
    }
    {
      ...
      EXPECT_NO_THROW(r = apply_transaction(bstore, &osr, std::move(t)));
      ASSERT_EQ(r, 0);
    }
    {
      ObjectStore::Transaction t;
...
TEST_P(StoreTest, BluestoreInlineCloneRangeTest) {
  ...
  {
    ObjectStore::Transaction t;
    t.write(cid, hoid, 10, 10, small);
    cerr << "inline write bad pattern" << hoid << std::endl;
    EXPECT_NO_THROW(r = apply_transaction(store, &osr, std::move(t)));
    ASSERT_EQ(r, 0);
  }
  {
    ObjectStore::Transaction t;
    t.clone_range(cid, hoid, hoid2, 0, 5, 10);
    cerr << "Clone range object" << std::endl;
    EXPECT_NO_THROW(r = apply_transaction(store, &osr, std::move(t)));
    ASSERT_EQ(r, 0);
  }
...
TEST_P(StoreTest, BluestoreInlineWritefullTest) {
  ...
  g_ceph_context->_conf->set_val("data_inline_limit_bytes", "10");
  g_ceph_context->_conf->set_val("debug_rocksdb", "22");
  g_ceph_context->_conf->set_val("debug_osd", "22");
  g_ceph_context->_conf->set_val("debug_bluestore", "22");
  g_ceph_context->_conf->apply_changes(NULL);
  ...
  small.append("small");
  {
    ObjectStore::Transaction t;
    t.touch(cid, hoid);
    t.set_alloc_hint(cid, hoid, 5, 5, CEPH_OSD_ALLOC_HINT_FLAG_INLINE);
    t.write(cid, hoid, 0, 5, small);
    cerr << "write full" << hoid << std::endl;
    r = apply_transaction(store, &osr, std::move(t));
    ASSERT_EQ(r, 0);
    ...
    bufferptr bp;
    r = store->getattr(cid, hoid, "idt", bp);
  ...
  { // inline -> inline_small
    ...
    t.set_alloc_hint(cid, hoid, 2, 2, CEPH_OSD_ALLOC_HINT_FLAG_INLINE);
    t.write(cid, hoid, 0, 2, smaller);
...
TEST_P(StoreTest, mergeRegionTest) {
  ...
  g_conf->set_val("bluestore_fsck_on_mount", "true");
  g_conf->set_val("bluestore_fsck_on_umount", "true");
  g_conf->set_val("bdev_debug_inflight_ios", "true");
  ...
  { // 5. overlapping
    ...
    bufferlist bl;
    r = store->read(cid, hoid, 0, final_len, bl);
